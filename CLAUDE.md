...
today uses the labelled fixture provider (scripted output). Benchmark: retrieval + static
checklist are real; see evals/REPORT.md for what is and is not measured.
...
One model adapter behind `src/server/llm/provider.ts`; Anthropic via plain `fetch`.
...
- `src/server/llm/` — `provider` (interfaces, errors), `anthropic`, `fixture/` (scripted demo + mechanical fallback), `generate` (retries/validation/cancel), `budget`, `contexts`, `prompts/`, `index` (provider selection).
...
```
npm install
npm run dev            # http://localhost:3000 (fixture provider unless ANTHROPIC_API_KEY set)
npm run lint           # eslint
...
(default `claude-sonnet-5`), `READYSPEC_ALLOWED_ROOTS` (path-delimited; fixtures always
allowed), `READYSPEC_DB`, `READYSPEC_MAX_CALLS|MAX_INPUT_TOKENS|MAX_OUTPUT_TOKENS|MAX_COST_USD`,
